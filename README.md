...
4. В основном цикле Вашей программы осуществляем запрос (*long polling*) происходящих с ботом событий, методом [get_updates](doc/get_updates.md). Результат помещаем в переменную, например upd = [get_updates()](doc/get_updates.md).
